...
vec3 CalcInterpolatedIndirectDiffuseLight (vec3 in_position, vec3 in_normal, vec2 texCoord)
{
	...
	if (distance (interpolatedPosition, in_position) < hgiMinInterpolationDistance
		...
		indirectDiffuseLight = texture (rsmIndirectDiffuseMap, texCoord).xyz;
	} else {
		if (hgiDebugInterpolation == 1) {
			indirectDiffuseLight = vec3 (1, 0, 0);
		} else {
			indirectDiffuseLight = CalcViewHGIIndirectDiffuseLight(in_position, in_normal) * hgiIntensity;
		}
	}
...
void main()
{
	...
	if (hgiInterpolationEnabled == 0) {
		indirectLight = CalcViewHGIIndirectDiffuseLight(in_position, in_normal) * hgiIntensity;
	}
