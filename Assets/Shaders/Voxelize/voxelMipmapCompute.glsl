...
layout(binding = 0) uniform sampler3D srcTexture;
layout(binding = 0, rgba8) uniform writeonly image3D dstImageMip;

vec4[8] fetchTexels(int dir, ivec3 pos) {
  return vec4[8] (texelFetch(srcTexture, pos + ivec3(1, 1, 1), SrcMipLevel),  // 0
                  texelFetch(srcTexture, pos + ivec3(1, 1, 0), SrcMipLevel),  // 1
                  texelFetch(srcTexture, pos + ivec3(1, 0, 1), SrcMipLevel),  // 2
                  texelFetch(srcTexture, pos + ivec3(1, 0, 0), SrcMipLevel),  // 3
                  texelFetch(srcTexture, pos + ivec3(0, 1, 1), SrcMipLevel),  // 4
                  texelFetch(srcTexture, pos + ivec3(0, 1, 0), SrcMipLevel),  // 5
                  texelFetch(srcTexture, pos + ivec3(0, 0, 1), SrcMipLevel),  // 6
                  texelFetch(srcTexture, pos + ivec3(0, 0, 0), SrcMipLevel)); // 7
}
